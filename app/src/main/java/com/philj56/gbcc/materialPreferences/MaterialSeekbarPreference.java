/*
 * This file is a copy of the androidx SeekBarPreference file,
 * modified to use a material slider instead. Note that the slider
 * works with floats, but seekbar works with integers, so some conversion
 * has to be done to keep preferences working the same.
...
public class MaterialSeekbarPreference extends Preference {
    ...
    private final Slider.OnChangeListener mSeekBarChangeListener = new Slider.OnChangeListener() {
        ...
        public void onValueChange(@NonNull Slider seekBar, float progress, boolean fromUser) {
            if (fromUser && (mUpdatesContinuously || !mTrackingTouch)) {
                ...
                updateLabelValue(Math.round(progress) + mMin);
            }
        }
    };
    ...
    private final Slider.OnSliderTouchListener mSeekBarTouchListener = new Slider.OnSliderTouchListener() {
        ...
        public void onStopTrackingTouch(Slider seekBar) {
            ...
            if (seekBar.getValue() + mMin != mSeekBarValue) {
                syncValueInternal(seekBar);
            }
        }
    };
    ...
    public void onBindViewHolder(@NonNull PreferenceViewHolder holder) {
        ...
        if (mSeekBarIncrement != 0) {
            ...
            mSeekBarIncrement = Math.round(mSeekBar.getStepSize());
        }
        mSeekBar.setValue(mSeekBarValue - mMin);
     ...
     */
    @SuppressWarnings("WeakerAccess") /* synthetic access */
    void syncValueInternal(Slider seekBar) {
        int seekBarValue = mMin + Math.round(seekBar.getValue());
        if (seekBarValue != mSeekBarValue) {
            if (callChangeListener(seekBarValue)) {
                setValueInternal(seekBarValue, false);
            } else {
                seekBar.setValue(mSeekBarValue - mMin);
                updateLabelValue(mSeekBarValue);
            }
        }
    }
